...
const char * const BITCOIN_CONF_FILENAME = "brofist.conf";
const char * const BITCOIN_PID_FILENAME = "brofistd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "brofist" is a composite category enabling all BroFist-related debug output
            if(ptrCategory->count(string("brofist"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BroFistCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BroFistCore
    // Mac: ~/Library/Application Support/BroFistCore
    // Unix: ~/.brofistcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BroFistCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BroFistCore";
#else
    // Unix
    return pathRet / ".brofistcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty brofist.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override brofist.conf
