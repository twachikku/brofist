 ...
 *
 *   // network
 *   CBroFistNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.brofist.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBroFistNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CBroFistNetwork(UniValue objIn)
    {
...
// // can be under: BroFistNetwork
// //   -- signature requirements : Key1(User)
// class CBroFistNetworkVariable : public CGovernanceObject
// {
// private:
